...
public class AmazonComDatasource extends AbstractDatasource {
	...
	protected void fillHeaderData(String fileContent) {
		...
		int startIndex = fileContent.indexOf(titleSearchPatternStart) + titleSearchPatternStart.length();
	...
	protected void traverseReviewData(String entryUrl) throws IOException {
		...
		int firstReviewpageUrlStartIndex = mainPageAsString.indexOf(reviewpageSearchPatternStart) + reviewpageSearchPatternStart.length();
		int firstReviewpageUrlEndIndex = mainPageAsString.indexOf(reviewpageSearchPatternEnd, firstReviewpageUrlStartIndex);
		String firstReviewPageUrl = mainPageAsString.substring(firstReviewpageUrlStartIndex, firstReviewpageUrlEndIndex);
	...
	protected void fillReviewData(String fileContent) {
		...
		while (dateStartIndex != -1) {
			...
			dateStartIndex = fileContent.indexOf(dateSearchPatternStart,
					dateSearchSpaceOffset);
			quantitySearchSpaceOffset = quantityEndIndex + quantitySearchPatternEnd.length();
